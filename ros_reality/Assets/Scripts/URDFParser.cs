        ...
        foreach (XmlNode joint in jointList) {
            ...
            foreach (XmlNode jointComp in jointComponents) {
                if (jointComp.Name == "origin") { //Found the visual component
                    ...
                    if (jointComp.Attributes["rpy"] != null)
                    {
                    ...
                    {
                        jo_rpy = new Vector3(0f, 0f, 0f);
                        jo_rpyF = true;
                    }

                    string[] xyzVals = jointComp.Attributes["xyz"].Value.Split(delims);
                    jo_xyz = new Vector3(float.Parse(xyzVals[0]), float.Parse(xyzVals[1]), float.Parse("-2"));
                    jo_xyzF = true;
                }
            ...
            {
                Debug.Log("[URDFParser] joint not added: " + jname);

                // jointDict.Add(jname, new Joint(jname, jtype, new Vector3(0f, 0f, 0f)
                //                             , new Vector3(0f, 0f, 0f), jparentLink, jchildLink));
            }
        }
