...
//					}
					...
					// expand vertices in view space so that they're always the same size no matter what direction you're looking at them from
					float4 viewPosExpanded = viewPos + (quadOffsets[0] * XYSpread * worldScaledDepth);
					output.pos = mul(UNITY_MATRIX_P, viewPosExpanded);
					triStream.Append(output);	


					// expand vertices in view space so that they're always the same size no matter what direction you're looking at them from
					viewPosExpanded = viewPos + (quadOffsets[1] * XYSpread * worldScaledDepth);
					output.pos = mul(UNITY_MATRIX_P, viewPosExpanded);
					triStream.Append(output);	


					// expand vertices in view space so that they're always the same size no matter what direction you're looking at them from
					viewPosExpanded = viewPos + (quadOffsets[2] * XYSpread * worldScaledDepth);
					output.pos = mul(UNITY_MATRIX_P, viewPosExpanded);
					triStream.Append(output);	


					// expand vertices in view space so that they're always the same size no matter what direction you're looking at them from
					viewPosExpanded = viewPos + (quadOffsets[3] * XYSpread * worldScaledDepth);
					output.pos = mul(UNITY_MATRIX_P, viewPosExpanded);
					triStream.Append(output);	
				}
