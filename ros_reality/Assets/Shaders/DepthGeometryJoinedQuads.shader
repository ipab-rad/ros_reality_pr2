		...
		Pass
		{
			...
			CGPROGRAM
				...
				void GS_Main(point EMPTY_INPUT p[1], uint primID : SV_PrimitiveID, inout TriangleStream<POSCOLOR_INPUT> triStream)
				{
					...
					// CHANGED: Unrolled for loop due to bug in Unity being unable to escape the for loop during compile time
					// for (int i = 0; i < 4; ++i)
					// {
					...
					worldPos.xy = float2(textureCoordinates.x, DepthHeight - textureCoordinates.y);		
					worldPos += quadOffsets[0];
					// Center coordinates such that 0,0 is the center in the world					
					worldPos.xy -= DepthHalfWidthHeight;			
					// Data is projected onto the 2D sensor, but knowing the depth and field of view we can reconstruct world space position
					worldPos.xy *= XYSpread * worldScaledDepths[0];					
					worldPos.z = worldScaledDepths[0];
					output.pos = mul(UNITY_MATRIX_VP, worldPos);
					triStream.Append(output);	


					worldPos.xy = float2(textureCoordinates.x, DepthHeight - textureCoordinates.y);
					worldPos += quadOffsets[1];
					worldPos.xy -= DepthHalfWidthHeight;			
					worldPos.xy *= XYSpread * worldScaledDepths[1];						
					worldPos.z = worldScaledDepths[1];	
					output.pos = mul(UNITY_MATRIX_VP, worldPos);
					triStream.Append(output);


					worldPos.xy = float2(textureCoordinates.x, DepthHeight - textureCoordinates.y);
					worldPos += quadOffsets[2];
					worldPos.xy -= DepthHalfWidthHeight;			
					worldPos.xy *= XYSpread * worldScaledDepths[2];					
					worldPos.z = worldScaledDepths[2];	
					output.pos = mul(UNITY_MATRIX_VP, worldPos);
					triStream.Append(output);	


					worldPos.xy = float2(textureCoordinates.x, DepthHeight - textureCoordinates.y);
					worldPos += quadOffsets[3];
					worldPos.xy -= DepthHalfWidthHeight;			
					worldPos.xy *= XYSpread * worldScaledDepths[3];					
					worldPos.z = worldScaledDepths[3];	
					output.pos = mul(UNITY_MATRIX_VP, worldPos);
					triStream.Append(output);
				}
